...
def overlaps(src: List[set], trg: List[set]):
    return np.array([[len(s.intersection(t)) for t in trg] for s in src])
...
class Metis:
    def __init__(self, data: EAData):
        ...
        self._data = data
        self._train_set, self._train_map,self._test_map, self._link_set, self._test_set = self.get_train_node_sets(data)
    ...
    def construct_edge_graph(triples, important_nodes=None, weight=1000):
        ...
        progress = tqdm(total=len(edges_map.items()))
    ...
    def construct_graph(triples, important_nodes=None, known_weight=1000, known_size=None,
        ...
        if cnt_as_weight:
            edges = Metis.make_cnt_edges(Metis.make_cnt_edges(edges))
    ...
    def subgraph_trainset(self, node_lists, src=0, no_trg=False):
        ...
        for i, nodes in enumerate(tqdm(node_lists)):
            curr = []
            for n in nodes:
                if n in train:
                    curr.append(n)
